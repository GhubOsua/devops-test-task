...
-c 10 – количество параллельных соединений (concurrency). Это означает, что ab будет поддерживать 10 одновременно открытых соединений,
отправляя запросы непрерывно, пока не наберётся 100 успешных запросов;
```
